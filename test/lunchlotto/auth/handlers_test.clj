...
(defmocktest register-user-test
  (testing "valid email format and email is not in database"
    (stubbing [models/find-user-by-email nil
      ...
      (with-redefs [handlers/email (->FakeEmail (atom []))]
        (let [{:keys [:status :headers :flash]} (handlers/register-user {:scheme "http" :server-name "localhost" :server-port "1234" :params {:email "jdoe@cyrusinnovation.com"}})]
          (is (= 302 status))
          (is (= "/" (get headers "Location")))
          (is (contains flash "You've just been sent an email"))
          (is (= {:text  "http://localhost:1234/confirm?confirmation_token=some-token"
                  :to    "jdoe@cyrusinnovation.com"}
                 (first @(:state handlers/email))))))))
...
(deftest update-confirmation-token
  (testing "successfully updated token"
    (with-redefs [models/update-confirmation-token (fn [_] "some-token")
      ...
      (let [resp (handlers/update-confirmation-token {:scheme "http" :server-name "localhost" :server-port "1234" :params {:email "jdoe@cyrusinnovation.com"}})]
        (is (= 302 (:status resp)))
